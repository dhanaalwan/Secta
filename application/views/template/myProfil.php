...
<?php foreach ($users->result() as $d): ?>
	<?php $Status = $this->session->userdata('Status') == 'TugasAkhir' ? 'Mahasiswa' : $_SESSION['Status'];?>
	<div class="card mb-1">
		<?php if ($d->Foto !== null) {
		...
		<form action="<?= base_url('ControllerGlobal/uploadFoto') ?>" method="post" class='form-inline' enctype="multipart/form-data">
			<input type="file" name="upload" id="upload_foto" class='form-control-sm' style='display:none' value="$foto['file_name']">	
