<head>
	<script type="text/javascript">
			...
			$(document).on('click', '.save', function(e) {
				...
				for (let i = 0; i < vlue.length; i++) {
					if (vlue[i] === vlue[2]) {
						...
						let idprogramstudi = vlue[i].value.split(";")
						$(this).parents('tr').siblings('tr#tabel'+idbutton).show().append(`<td>${idprogramstudi[1]}</td>`);

					} else if (vlue[i] === vlue[3]) {

						let idbidangminat = vlue[i].value.split(";")
						$(this).parents('tr').siblings('tr#tabel'+idbutton).show().append(`<td>${idbidangminat[1]}</td>`);
				...
				const programstudi = vlue[2].value.split(";")[0]
				const bidangminat = vlue[3].value
				...
				$.ajax({
					...
					type: 'post',
					dataType: 'json',
					data: {id, name, programstudi, bidangminat, email, nohp, idlama: idbutton},
				})
				.done(function() {
					console.log("success");
				})
		...
		$(document).ready(function() {
			...
			$("#tabelProgramStudiAdmin").load('<?= base_url('admin/tabelProgramStudiAdmin'); ?>');
			...
			$("#btn_jrsn").click(function() {
				$("#form_bidangminat").fadeOut('slow', function() {
					$("#form_programstudi").fadeIn('fast');			
					$("#form_programstudi").load('<?php echo base_url('admin/formProgramStudi'); ?>');	
				});
			});

			$("#btn_bidangminat").click(function() {
				$("#form_programstudi").fadeOut('slow', function() {
					$("#form_bidangminat").fadeIn('fast');
					$("#form_bidangminat").load('<?php echo base_url('admin/formBidangMinat'); ?>');	
				});
			});
	...
	<div class="row">
		<div class="col-xl-2 menu">
			<div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
				...
				<a class="nav-link <?= $bidangminat ? '' : 'disabled' ?>" id="pills-mahasiswa" data-toggle="pill" href="#pills-tabel-mahasiswa" role="tab" aria-controls="pills-tabel-mahasiswa" aria-selected="false"><i class="fas fa-graduation-cap"></i> Mahasiswa</a>
				
				<a class="nav-link <?= $bidangminat ? '' : 'disabled' ?>" id="v-pills-dosen-tab" data-toggle="pill" href="#v-pills-dosen" role="tab" aria-controls="v-pills-dosen" aria-selected="false"><i class="fas fa-briefcase"></i> Dosen </a>
				...
				<a class="nav-link" id="navPanduan" data-toggle="pill" href="#list-panduan" role="tab" aria-controls="panduan" aria-selected="true"><i class="fas fa-help"></i> Panduan</a>
				<br>
				<br>
			</div>
		</div>
			...
			<div class="tab-content" id="v-pills-tabContent">
						...
						<div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
							<div class="form-row">
								<div class="form-group col-md">
									<button id="btn_jrsn" class="btn btn-sm btn-primary col text-left"> <i class="fas fa-user-plus"></i> Program Studi </button>								
								</div>

								<div id="form_programstudi" class="col-md-9" style="display: none">

								</div>
							</div>
							<div class="form-row">
								<div class="form-group col-md">
									<button id="btn_bidangminat" class="btn btn-primary btn-sm col text-left"> <i class="fas fa-user-plus"></i> Bidang Minat </button>
								</div>

								<div class="col-md-9" id="form_bidangminat" style="display: none">

								</div>									
							</div>
						</div>
						<div id="tabelProgramStudiAdmin">
				...
				<div class="tab-pane fade" id="list-panduan" role="tabpanel" aria-labelledby="list-panduan-list">
					<div class="row">
						...
						<div class="col col-md">
							<h5 class="text-right nav-link"> PANDUAN </h5>
						</div>
					</div>
					<hr>
